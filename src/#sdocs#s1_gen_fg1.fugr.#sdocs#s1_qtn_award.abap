...
select single config_value into mv_final_url from (lv_db_table) where config = 'SPLR_QTN_AWARD'.
  replace '{ p_qtn }' into mv_final_url with p_qtn.
*}   INSERT
*{   DELETE         S4HK903118                                        1
*\data(MV_FINAL_URL) = |https://VHCALS4HCS.DUMMY.NODOMAIN:443/sap/opu/odata/sap/API_QTN_PROCESS_SRV/SubmitForApproval?SupplierQuotation='{ p_qtn }'|.
*}   DELETE
