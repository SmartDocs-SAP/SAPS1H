FUNCTION /SDOCS/S1_RFQ_APPR_PUBLISH.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
...
select single CONFIG_VALUE from (lv_db_table) into mv_final_url where config = 'RFQ_PUBLISH'.
  Replace '{ p_rfq }' IN mv_final_url WITH p_rfq.
*}   INSERT

*{   DELETE         S4HK903118                                        1
*\     data(MV_FINAL_URL) = |https://VHCALS4HCS.DUMMY.NODOMAIN:443/sap/opu/odata/sap/API_RFQ_PROCESS_SRV/SubmitForApproval?RequestForQuotation='{ p_rfq }'|.
*}   DELETE
